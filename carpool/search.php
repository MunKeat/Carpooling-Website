        ...
        <div class="row">
            <div class="large-4 primary-background-translucent full-length columns">
                <div class="large-12 text-center columns">
                    ...
                    if(isset($_POST['search']) && $parameters_set === True) {
                        // Get statistic relating to search query
                        $stmt = "SELECT COUNT(*) AS AVAIL_CARPOOL, MIN(TRIP_TIME) AS MIN_TRIP_TIME, MAX(TRIP_TIME) AS MAX_TRIP_TIME, MIN(COST) AS MIN_COST, MAX(COST) AS MAX_COST FROM SEARCHQUERY
                        WHERE DEPARTURE = :departure AND DESTINATION = :destination AND TRIP_DATE = :date";
                        // Prepare statement
                        $query = oci_parse($connect, $stmt);
                        // Bind variables to placeholders
                        oci_bind_by_name($query, ":departure", $departure);
                        oci_bind_by_name($query, ":destination", $destination);
